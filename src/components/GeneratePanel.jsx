  ...
  const generate = async () => {
    ...
    try {
      if (tab === "image") {
        ...
        throw new Error(`${tab} generation requires a third-party API key. Configure in Identity panel.`);
      }
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 flex flex-col gap-4">
        <div>
          ...
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)}
            placeholder={tab === "image" ? "Describe the image in detail..." : `Describe the ${tab}...`}
            rows={4} className="w-full rounded-lg px-3 py-2.5 text-sm text-gray-200 placeholder-gray-600 font-mono resize-none outline-none"
        ...
        {error && (
          <div className="rounded-lg p-3 text-xs font-mono text-red-400" style={{ background: "#160808", border: "1px solid #3b0000" }}>{error}</div>
        )}

        {result?.type === "image" && (
          <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #1a1a2e" }}>
            <div className="flex items-center justify-between px-3 py-2" style={{ background: "#080812", borderBottom: "1px solid #1a1a2e" }}>
              <span className="text-xs font-mono text-green-500">● GENERATED</span>
            ...
            {result.revised && <p className="text-xs text-gray-600 font-mono p-3 leading-relaxed">{result.revised}</p>}
          </div>
        )}
      </div>
    </div>
