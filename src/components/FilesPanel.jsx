...
export default function FilesPanel() {
  ...
  const processFiles = (fileList) => {
    Array.from(fileList).forEach(file => {
      ...
      setFiles(prev => [...prev, { id: Date.now() + Math.random(), name: file.name, size: (file.size/1024).toFixed(1)+" KB", type: file.type||"unknown", url }]);
    });
  };
  ...
  return (
    <div className="flex h-full">
      <div className="flex flex-col w-64 flex-shrink-0" style={{ borderRight: "1px solid #1a1a2e" }}>
        <div onDrop={e => { e.preventDefault(); setDragging(false); processFiles(e.dataTransfer.files); }}
          onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
          onClick={() => fileRef.current?.click()}
          className="m-3 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer flex-shrink-0"
              ...
              return (
                <div key={f.id} onClick={() => setPreview(f)}
                  ...
                  <button onClick={e => { e.stopPropagation(); setFiles(fs=>fs.filter(x=>x.id!==f.id)); if(preview?.id===f.id)setPreview(null); }}
                    className="text-gray-700 hover:text-red-500"><X size={11} /></button>
                </div>
              );
            })}
        </div>
        ...
        {preview ? (
          <>
            ...
            <div className="flex-1 overflow-auto p-4 flex items-start justify-center">
              {preview.type.startsWith("image") && <img src={preview.url} alt={preview.name} className="max-w-full max-h-full rounded-lg object-contain" />}
              {preview.type.startsWith("audio") && <div className="w-full mt-8"><audio controls className="w-full"><source src={preview.url} /></audio></div>}
              {preview.type.startsWith("video") && <video controls className="max-w-full rounded-lg" style={{ maxHeight:"70vh" }}><source src={preview.url} /></video>}
              {!preview.type.startsWith("image") && !preview.type.startsWith("audio") && !preview.type.startsWith("video") && (
                <div className="text-center mt-16">
                  ...
                  <a href={preview.url} download={preview.name} className="inline-block mt-4 px-4 py-2 rounded text-xs font-mono text-blue-400 border border-blue-900">DOWNLOAD</a>
                </div>
              )}
            </div>
          </>
