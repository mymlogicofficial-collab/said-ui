...
export default function TerminalPanel() {
    ...
    switch (base) {
      case "help":
        push(
          ...
          { t: "out", v: "  date          — current date" },
          { t: "out", v: "  whoami        — current operator" },
          { t: "out", v: "  echo [text]   — print text" },
          { t: "out", v: "  run [expr]    — evaluate JS expression" },
          { t: "out", v: "  memory        — show memory stats" },
          { t: "out", v: "  skills        — list skills" },
          { t: "out", v: "  identity      — show identity config" },
      ...
      case "said":
        push(
          { t: "out", v: "  S.A.I.D. — Synthetic Adaptive Intelligence Designer" },
          { t: "out", v: "  Strategized · Analyzed · Improvised · Designs" },
          { t: "out", v: "  © MYM Logic LLC — All Rights Reserved" },
          { t: "blank" },
        );
        break;
      case "ls":
        push(
          { t: "out", v: "drwxr-xr-x  sandbox/" },
          { t: "out", v: "drwxr-xr-x  memory/" },
          { t: "out", v: "drwxr-xr-x  skills/" },
          { t: "out", v: "drwxr-xr-x  identity/" },
          { t: "out", v: "drwxr-xr-x  app-editor/" },
          { t: "out", v: "-rw-r--r--  said.config.json" },
          { t: "blank" },
        );
        break;
      case "date":
        push({ t: "out", v: new Date().toString() }, { t: "blank" });
        break;
      case "whoami":
        push({ t: "out", v: "operator@said-system" }, { t: "blank" });
        break;
      case "echo":
        push({ t: "out", v: args }, { t: "blank" });
        break;
      case "run":
        try {
          ...
          push({ t: "err", v: `Error: ${e.message}` }, { t: "blank" });
        }
        break;
      case "memory": {
      ...
      case "identity": {
        ...
        push(
          { t: "out", v: `  Name:    ${id.name || "Not set"}` },
          { t: "out", v: `  Model:   ${id.model || "gpt-4o"}` },
          { t: "out", v: `  API Key: ${id.apiKey ? "●●●●●●●●●●●●" : "Not configured"}` },
          { t: "blank" },
        );
        break;
      }
      case "clearchat":
        localStorage.removeItem("said_chat");
        push({ t: "out", v: "Chat history cleared." }, { t: "blank" });
        break;
      case "clearmem":
        localStorage.removeItem("said_memory");
        push({ t: "out", v: "Memory cleared." }, { t: "blank" });
        break;
      default:
        push({ t: "err", v: `said: command not found: ${base}` }, { t: "blank" });
    }
  };
  ...
  const colors = { sys: "#3b82f6", out: "#00ff88", err: "#ef4444", prompt: "#60a5fa", blank: "transparent" };
  ...
  return (
    <div className="flex flex-col h-full cursor-text" style={{ background: "#050508" }}
      ...
      <div className="flex items-center gap-2 px-4 py-2 flex-shrink-0"
        ...
        <span className="text-xs font-mono text-green-800 ml-2 tracking-widest">SAID TERMINAL — bash</span>
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-3" style={{ background: "#050508" }}>
        {lines.map((line, i) => (
